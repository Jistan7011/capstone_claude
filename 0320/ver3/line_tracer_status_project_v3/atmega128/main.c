...
ISR(USART0_RX_vect) {
    ...
    if (!is_auto_mode && received_char >= '0' && received_char <= '9') {
...
static const char* uid_to_zone(const uint8_t* uid, uint8_t len) {
    ...
    if (uid[0]==0xE1 && uid[1]==0xCE && uid[2]==0x2D && uid[3]==0xFF) return "A구역";
    if (uid[0]==0x00 && uid[1]==0x82 && uid[2]==0xC2 && uid[3]==0x2C) return "B구역";
    if (uid[0]==0xB1 && uid[1]==0xE7 && uid[2]==0x32 && uid[3]==0xFF) return "C구역";
    if (uid[0]==0x01 && uid[1]==0xCE && uid[2]==0x2D && uid[3]==0xFF) return "D구역";
    return NULL;
}
...
static void send_status_to_bluetooth(void) {
    ...
    dtostrf(right_rpm, 4, 1, rbuf);
    sprintf(buf,
        "[STATUS] MODE:%s DIR:%s RPM L:%s R:%s ZONE:%s LOCATION:%s AGE:%u SPD:%d\r\n",
        is_auto_mode ? "AUTO" : "MANUAL",
        current_direction,
        lbuf,
        rbuf,
